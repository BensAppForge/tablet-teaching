...
const HorizontalReorderingEditor: React.FC<HorizontalReorderingEditorProps> = ({
  ...
  // Stable opaque id per row. dnd-kit uses these as sortable identity;
  // if we used `item` text directly, duplicate or empty strings (very
  // likely from AI output or a freshly added row) would collide and the
  // drag would visually swap but snap back on drop.
  ...
  // Sensors: distance constraint for mouse (allows clicks), short delay
  // for touch (lets the teacher tap into Input fields without starting
  // a drag), keyboard sensor for accessibility.
  ...
  // id), bring rowIds back in line so SortableContext has matching ids.
  useEffect(() => {
